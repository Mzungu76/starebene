...
import { exportBackup, importBackup, type AppBackup, type LegacyAppBackup } from "@/lib/indexeddb";
...
export function BackupPanel() {
  ...
  async function onImport(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const parsed = JSON.parse(text) as AppBackup | LegacyAppBackup;

      const validLegacy = parsed.version === 1 && Array.isArray(parsed.profile) && Array.isArray(parsed.checkIns);
      const validCurrent =
        parsed.version === 2 &&
        Array.isArray(parsed.profile) &&
        Array.isArray(parsed.checkIns) &&
        Array.isArray(parsed.mealPlanWeeks);
